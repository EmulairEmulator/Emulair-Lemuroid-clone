...
class SaveSyncManagerImpl(
    ...
    override fun getLastSyncInfo(): String {
        ...
        return appContext.getString(R.string.save_sync_synchronization_last_sync_completed, dateString)
    }

    override fun getConfigInfo(): String {
        val email = GoogleSignIn.getLastSignedInAccount(appContext)?.email
        return if (email != null) {
            appContext.getString(R.string.save_sync_gdrive_connected_summary, email)
        } else {
            appContext.getString(R.string.save_sync_gdrive_connected_none_summary)
        }
    }
