...
class ActivateGoogleDriveActivity : Activity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (errorCode == ConnectionResult.SUCCESS) {
            ...
            val message = getString(R.string.save_sync_gdrive_missing_play_services)
    ...
    override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
        ...
        if (requestCode == REQUEST_GOOGLE_SIGN_IN) {
            ...
            try {
                ...
                val message = getString(R.string.save_sync_gdrive_sign_in_success, account?.email)
                Toast.makeText(this, message, Toast.LENGTH_SHORT).show()
                finish()
            } catch (e: ApiException) {
                val message = getString(
                    R.string.save_sync_gdrive_sign_in_failed,
                    e.message,
    ...
    private fun disableGoogleDriveIntegration() {
        ...
        signInClient.signOut().addOnSuccessListener {
            Toast.makeText(this, R.string.save_sync_gdrive_sign_out_success, Toast.LENGTH_SHORT).show()
            finish()
        }
    }
