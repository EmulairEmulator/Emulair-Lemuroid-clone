package com.bigbratan.emulair.fragmentSystemGames
...
class SystemGameViewHolder(parent: View) : RecyclerView.ViewHolder(parent) {
...
class SystemGamesAdapter(
    private val baseLayout: Int,
    private val gameInteractor: GameInteractor,
    private val coverLoader: CoverLoader
) : PagingDataAdapter<Game, SystemGameViewHolder>(Game.DIFF_CALLBACK) {

    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): SystemGameViewHolder {
        return SystemGameViewHolder(LayoutInflater.from(parent.context).inflate(baseLayout, parent, false))
    }

    override fun onBindViewHolder(holder: SystemGameViewHolder, position: Int) {
        getItem(position)?.let { holder.bind(it, gameInteractor, coverLoader) }
    }

    override fun onViewRecycled(holder: SystemGameViewHolder) {
