...
class InputBindingUpdater(private val inputDeviceManager: InputDeviceManager, intent: Intent) {
    ...
    fun getTitle(context: Context): String {
        val keyName = GamePadPreferences.displayNameForKeyCode(extras.retroKey)
        return context.getString(R.string.gamepad_retropad_binding_popup_title, keyName)
    }

    fun getMessage(context: Context): String {
        return context.getString(R.string.gamepad_retropad_binding_popup_description, extras.device.name)
    }
